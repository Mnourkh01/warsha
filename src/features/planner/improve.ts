...
export function buildImprovePrompt(doc: PlanDoc, improvements: string[]): string {
  ...
  return [
    ...
    "- Allowed kinds: phase | task | api | service | data | note.",
    "- api blocks may set method (GET|POST|PUT|PATCH|DELETE) and path; task blocks may set acceptance (string[]); data blocks may set fields ([{name, type, note?}]).",
