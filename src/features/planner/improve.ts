...
export function buildImprovePrompt(doc: PlanDoc, improvements: string[]): string {
  ...
  return [
    ...
    "- api blocks may set method (GET|POST|PUT|PATCH|DELETE) and auth (public|user|admin); api and screen blocks may set path; ai and agent blocks may set model (string); service/ai/agent/integration/data/deploy blocks may set spec (one line: technology / input-to-output contract / exit condition / provider / primary key / rollback plan); note blocks may set flavor (idea|risk|question|constraint); data blocks may set sensitivity (none|personal|sensitive) and fields ([{name, type, note?}]); test blocks may set testType (unit|integration|e2e|manual); deploy blocks may set env (dev|staging|prod); decision blocks may set chosen (one of its options); task, decision, test, agent, phase, and screen blocks may set acceptance (string[]: acceptance criteria, options, checks, tools, exit criteria, or screen states); any block may set status (doing|done), effort (s|m|l), priority (must|should|could), owner (string), due (string), and link (https url).",
